...
function RefreshRadioResults(isNext, isNextPrev) {
    ...
    if (RadioDateValidation()) {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: _urlTAdsSelectRadioStationResults,
            contentType: 'application/json; charset=utf-8',
            ...
            success: function (result) {
                ...
                if (result != null && result.isSuccess) {
                    ...
                    if (result.FacetFilters != null) {
                       updateSideFilters(result.FacetFilters);
                     }
                }
                else {
...
function updateSideFilters(filters) {
    var iqdmalist = "<option value=\"0\"></option>";
    if (filters.RadioMarket != null) {
        $.each(filters.RadioMarket, function (index, obj) {
            var selected = "";
            if (_MarketName.includes(obj.Name)) { selected = "selected"; }
            iqdmalist = iqdmalist + '<option ' + selected + ' value=\"' + obj.Name + '\">' + obj.Name + '(' + obj.CountFormatted + ')</option>';
        });
        $("#radioMarketSelect").html(iqdmalist);
    } else {
        iqdmalist += "<option value=\"0\" disabled=\"true\">No Filter Available</option>";
        $("#radioMarketSelect").html(iqdmalist);
    }
    updateChosenSelect(iqdmalist, "#radioMarketSelect");
    var iqstationlist = "<option value=\"0\" ></option>";
    if (filters.RadioStation != null) {
        $.each(filters.RadioStation, function (index, obj) {
            var selected = "";
            if (_RadioStation.includes(obj.ID)) { selected = "selected"; }
            var liStyle = "";
            iqstationlist = iqstationlist + '<option ' + selected + ' value=\"' + obj.ID + '\">' + obj.Name + '(' + obj.CountFormatted + ')</option>';
        });
        $("#radioStationSelect").html(iqstationlist);
    } else {
        iqstationlist = "<option value=\"0\" disabled=\"true\">No Filter Available</option>";
        $("#radioStationSelect").html(iqstationlist);
    }
    updateChosenSelect(iqstationlist, "#radioStationSelect");
}
