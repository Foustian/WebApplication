var _Date = null;
var _Station = "";
var _urlKantorMediaSearchResults = "/QVC/QVCSearchResults/"
var _urlKantorMediaSearchResultsPaging = "/QVC/QVCSearchResultsPaging/"
var _urlKantorMediaGetChart = "/QVC/GetChart/"
var _SearchTerm = "";
...
function SearchResult() {
    ...
    if (ValidateSearch()) {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: _urlKantorMediaSearchResults,
...
function SearchResultPaging(isNextPage) {
    ...
    if (ValidateSearch()) {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: _urlKantorMediaSearchResultsPaging,
            contentType: 'application/json; charset=utf-8',
            data: JSON.stringify(jsonPostData),
            success: OnResultSearchComplete,
            error: OnFail
...
function OnResultSearchComplete(result) {
    ...
    if (result.isSuccess) {
        ...
        $('#lblRecords').html('');
        $('#ulKantorResults').html('');
        $('#ulKantorResults').html(result.HTML);
...
function ValidateSearch() {
    ...
    if (_Station == null || _Station == "") {
        isValidate = false;
        ShowNotification("Please Select Station");
        
    }

    if (_Date == null || _Date=="") {
        isValidate = false;
        ShowNotification("Please Select Date");
    }
...
function OnGetChartComplete(result) {
    if (result.isSuccess) {
        ...
        // Only show Audience series by default
        $($("#divKantorChart2").highcharts().series).each(function (i, e) {
            if (e.name != "Audience") {
                e.hide();
            }
        });
...
function RenderHighCharts(jsonLineChartData) {
    ...
    JsonLineChart.legend.x = 0;
    JsonLineChart.legend.align = 'center';
    ...
    if (!_IsManualHover) {
        ...
        for (var seriesIndex = 0; seriesIndex < chart.series.length; seriesIndex++) {
            if (chart.series[seriesIndex].visible) {
                chart.series[seriesIndex].data[xIndex].setState('hover');
                seriesTooltip.push(chart.series[seriesIndex].data[xIndex]);
            }
            else {
                chart.series[seriesIndex].data[xIndex].setState('');
            }
        }

        if (seriesTooltip.length>0) {
            chart.tooltip.refresh(seriesTooltip);
        }
        else {
            chart.tooltip.hide();
        }
...
//        }
        ...
        str += '<br/><span style="color:' + point.series.color + ';font-weight:bold;">' + point.series.name + ' ' + point.series.data[point.key].Value + '</span>';
    });
    return str;
}
